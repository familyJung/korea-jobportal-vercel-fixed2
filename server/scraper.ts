...
async function scrapeJobsFromUrl(url: string, ministryName: string): Promise<ScrapedJob[]> {
  try {
    ...
    // 고용노동부 디버깅
    if (ministryName === '고용노동부') {
      console.log(`🔍 [DEBUG] ${ministryName} - Final jobs found: ${jobs.length}`);
      
      // HTML 전체에서 [인사] 라벨 확인
      const allText = $.text();
      const hasPersonnelLabel = allText.includes('[인사]');
      console.log(`🔍 [DEBUG] Page contains [인사] label: ${hasPersonnelLabel}`);
      
      if (hasPersonnelLabel) {
        console.log(`🔍 [DEBUG] Searching for [인사] labeled content...`);
        
        // 테이블 행에서 [인사] 찾기
        $('tr').each((index, row) => {
          const rowText = $(row).text();
          if (rowText.includes('[인사]')) {
            console.log(`🔍 [DEBUG] Found [인사] in row ${index}: ${rowText.substring(0, 150)}`);
          }
        });
        ...
        $('a').each((index, link) => {
          ...
          if (linkText.includes('[인사]')) {
            console.log(`🔍 [DEBUG] Found [인사] in link: ${linkText}`);
          }
        });
        ...
        $('td').each((index, cell) => {
          ...
          if (cellText.includes('[인사]')) {
            console.log(`🔍 [DEBUG] Found [인사] in cell: ${cellText}`);
          }
        });
      }
    }
